...
function attachTabEvents(userUid) {
    document.querySelectorAll('.profile-tab').forEach(tab => {
        tab.addEventListener('click', async () => {
            ...
            if (tabName === 'bookmarks')
                await loadBookmarks(userUid);
            if (tabName === 'reviews')
                await loadUserReviews(userUid);
            if (tabName === 'usefuls')
                await loadUserUsefuls(userUid);
        });
    });
}
async function loadBookmarks(userUid) {
...
async function loadUserReviews(userUid) {
    ...
    attachReviewCardEvents(el, () => { }, () => { }, async (reviewUid) => {
...
async function loadUserUsefuls(userUid) {
