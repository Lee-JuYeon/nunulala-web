...
import type { PlaceFilters } from '../types/models';
...
let currentFilters: PlaceFilters = { page: 1, limit: 20, sort: 'popularity' };
let currentPage = 1;
let totalPages = 1;

export async function renderPlaces(): Promise<void> {
  ...
  content.innerHTML = `
    <div class="places-toolbar">
      ...
      <div class="chip-group" id="type-chips">
        ${PLACE_TYPES.map(t =>
          `<button class="chip${currentFilters.type === t.value ? ' active' : ''}" data-type="${t.value}">${t.label}</button>`
        ).join('')}
      </div>
...
function attachToolbarEvents(): void {
  const searchInput = document.getElementById('place-search') as HTMLInputElement | null;
  ...
  chipGroup?.addEventListener('click', (e) => {
    const btn = (e.target as HTMLElement).closest('[data-type]') as HTMLElement | null;
    if (!btn) return;
...
async function loadPlaces(): Promise<void> {
  ...
  if (countEl) countEl.textContent = `총 ${meta.total}개의 장소`;
