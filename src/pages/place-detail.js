...
export async function renderPlaceDetail(params) {
  ...
  `;
    ...
    if (isLoggedIn()) {
        ...
        if (bkRes.success)
            bookmarked = bkRes.data.bookmarked;
        if (starRes.success)
            userStar = starRes.data.star ?? 0;
    }
...
function renderDetailPage(container, place, reviews, bookmarked, userStar) {
    ...
    ${loggedIn ? `
      <div class="place-star-container">
        ...
        <div class="stars" id="user-star-input">
          ${Array.from({ length: 5 }, (_, i) => `<span class="star${i < userStar ? ' filled' : ''}" data-value="${i + 1}" role="button" tabindex="0">★</span>`).join('')}
        </div>
      </div>
    ` : ''}
...
function renderReviewList(reviews) {
    ...
    return reviews.map(r => renderReviewCard(r, usefulSet.has(r.review_uid), () => { }, undefined, undefined)).join('');
}
function attachDetailEvents(placeUid, reviews, bookmarked, userStar) {
    ...
    document.getElementById('write-review-btn')?.addEventListener('click', () => {
        ...
        attachReviewFormEvents(async (data) => {
            const res = await api.createReview(placeUid, data);
            if (!res.success) {
                showToast('리뷰 작성에 실패했어요', 'error');
                return;
            }
...
function attachReviewListEvents(placeUid, reviews) {
    const list = document.getElementById('reviews-list');
    if (!list)
        return;
    attachReviewCardEvents(list, async (reviewUid) => {
        if (!isLoggedIn()) {
            navigate('/app/login');
            return;
        }
