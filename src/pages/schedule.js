...
function attachScheduleListEvents(schedules) {
    const list = document.getElementById('schedule-list');
    if (!list)
        return;
    list.addEventListener('click', async (e) => {
        const el = e.target;
        const editBtn = el.closest('.edit-schedule-btn');
        if (editBtn) {
            e.stopPropagation();
        ...
        const deleteBtn = el.closest('.delete-schedule-btn');
        if (deleteBtn) {
            e.stopPropagation();
...
function showScheduleForm(schedule) {
  ...
  `;
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const d_day = document.getElementById('sched-date').value;
...
async function showScheduleDetail(uid) {
    const content = document.getElementById('app-content');
    if (!content)
        return;
    content.innerHTML = `
